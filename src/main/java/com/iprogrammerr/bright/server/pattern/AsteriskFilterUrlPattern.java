...
public final class AsteriskFilterUrlPattern implements FilterUrlPattern {
	...
	public boolean isPrimary() {
		return this.urlPattern.startsWith(MATCH_ALL);
	}
	...
	public boolean isMatched(String url) {
		...
		if (this.urlPattern.endsWith(URL_SEGMENTS_SEPARATOR)
				&& urlSegments.length >= urlPatternSegments.length) {
			...
			for (int i = 0; i < urlPatternSegments.length; i++) {
				if (!urlPatternSegments[i].equals(MATCH_ALL)
