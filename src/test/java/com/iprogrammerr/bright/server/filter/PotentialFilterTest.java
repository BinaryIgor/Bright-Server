...
public final class PotentialFilterTest {
	...
	public void canMatchOneRuleToManyPatterns() {
		ConditionalFilter filter = new PotentialFilter(
				new ListOfRequestMethodRule(new GetMethod(), new PostMethod()), MOCKED_FILTER,
				"user/*", "game/*/search");
	...
	public void canRefuseMatches() {
		...
		ConditionalFilter filter = new PotentialFilter(new ListOfRequestMethodRule(get, post),
		...
		assertFalse(filter.areConditionsMet(request));
		filter = new PotentialFilter(MOCKED_FILTER,
